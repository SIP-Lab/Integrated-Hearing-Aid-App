...
ForwardFFT(Transform* fft, float* realInput, float calibration)
{
	...
	for (i=0; i<k; i++)
	{
        ...
        fft->totalPower += fft->power[i]/fft->points; //Previous VAD or Noise Classifier App did not divide with fft->points
                                                      //That's the reason power in SPL (the section below) was found higher.
	}
